...
public class CaissierController {
	...
	public void rechercher(String val) {
		...
		clImpl.list().stream().filter((x) -> x.getNom().contains(val) || x.getPrenom().contains(val))
				.forEach(tabClient.getItems()::add);
	}
	...
	void btnAction(ActionEvent event) {
		if (event.getSource() == btnRechercher) {
			...
			if (cbxVoiture.getSelectionModel().isEmpty() || txtNom.getText().isEmpty() || txtId.getText().isEmpty()
					|| txtPermi.getText().isEmpty() || txtPiece.getText().isEmpty() || txtPrenom.getText().isEmpty()
				...
				try {
					...
					Location lo = new Location(Integer.parseInt(txtId.getText()), voiture.getId(), format.format(dateD),
							format.format(dateR), Integer.parseInt(txtPrix.getText()));
	...
	void click(MouseEvent event) {
		if (!tabClient.getSelectionModel().isEmpty()) {
			...
			client = new Client(tabClient.getSelectionModel().getSelectedItem().getId(),
					tabClient.getSelectionModel().getSelectedItem().getNom(),
					tabClient.getSelectionModel().getSelectedItem().getPrenom(),
					tabClient.getSelectionModel().getSelectedItem().getPiece(),
					tabClient.getSelectionModel().getSelectedItem().getPermi());
