	...
	void click(ActionEvent event) {
		if (event.getSource() == btnModifier) {
			if (txtLogin.getText().isEmpty() || txtPassword.getText().isEmpty()
					|| cbxRole.getSelectionModel().isEmpty()) {
				alerter("veuillez remplir les champs", "information", 'w');
			} else {
				AuthentificationImpl auimpl = new AuthentificationImpl();
				au = new Authentification(Integer.parseInt(txtId.getText()), txtLogin.getText().trim(),
						txtPassword.getText().trim(), cbxRole.getSelectionModel().getSelectedItem().trim());
